...
For example, here example figure.png was imported.

![](images/shiny2.png){fig-align="center" width="280"}

The following steps were followed: **Orientate Figure → Calibrate Axes → Extract Data**. Upon completion, a message was displayed in the R console.

### Data extraction from community composition (ordination plot)
...
```{r}
# Copy df table from excel sheet
# Import df data from clipboard
df<- read.csv(file = "Data_folder/ordination_point.csv", header = TRUE, row.names = 1)
...
comparison<- read.csv(file = "Data_folder/ordination_point_category.csv", header = TRUE, row.names = 1)
...
From the results displayed in the R console, the relevant data were copied. These data are presented in Supplementary Table S3. These steps were repeated for all relevant studies, and the final datasets were obtained `Data_folder\community_structure\composition.csv` and `Data_folder\community_structure\beta_diversity.csv`.
...
### Map plus restoration strategies, past disturbance types and ecosystem
...
combined_plot_ref6 <- grid.arrange(Figure_2_ref4, Figure_2_ref5, ncol = 2, widths = c(1, 1), top = textGrob("Restored vs Reference", gp = gpar(fontsize = 12, fontface = "bold")))
Figure_2_ref6
```
...
```{r}
ggsave("figure_4.1.jpg", time_plot, dpi = 600, height = 9, width = 8)
```
...
```{r}
ggsave("figure_4.2.jpg", time_plot1, dpi = 600, height = 9, width = 8)
```
